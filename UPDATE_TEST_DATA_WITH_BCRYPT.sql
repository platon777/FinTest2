-- ============================================================================
-- SCRIPT DE MISE À JOUR COMPLÈTE - DONNÉES DE TEST AVEC BCRYPT
-- À exécuter sur Azure SQL Database
-- ============================================================================

PRINT '=================================================================';
PRINT 'DÉBUT NETTOYAGE ET RECRÉATION DONNÉES DE TEST';
PRINT '=================================================================';

-- ============================================================================
-- PARTIE 1 : NETTOYAGE DES DONNÉES EXISTANTES
-- ============================================================================

PRINT 'Nettoyage des données de test existantes...';

-- Supprimer dans l'ordre inverse des dépendances
DELETE FROM JournalAudit WHERE ClientID IN (
    SELECT ClientID FROM ClientsAuthentification
    WHERE Email IN ('marceus.jethro@email.ht', 'dr.patrick.marcellus@clinic.ht',
                     'joseph.woldy@university.edu.ht', 'alexandra.dorcean@archidesign.ht',
                     'contact@hoteloasis.ht', 'marie.jethro@designs.ht', 'sophia.marcellus@student.ht')
);

DELETE FROM VerificationsConformite WHERE ClientID IN (
    SELECT ClientID FROM ClientsAuthentification
    WHERE Email IN ('marceus.jethro@email.ht', 'dr.patrick.marcellus@clinic.ht',
                     'joseph.woldy@university.edu.ht', 'alexandra.dorcean@archidesign.ht',
                     'contact@hoteloasis.ht', 'marie.jethro@designs.ht', 'sophia.marcellus@student.ht')
...
DELETE FROM Transactions WHERE CompteSource IN (
    SELECT CompteID FROM Comptes WHERE NumeroCompte LIKE 'INV-202%' OR NumeroCompte LIKE 'SVG-202%'
) OR CompteDestination IN (
    SELECT CompteID FROM Comptes WHERE NumeroCompte LIKE 'INV-202%' OR NumeroCompte LIKE 'SVG-202%'
);

DELETE FROM Souscriptions WHERE CompteID IN (
    SELECT CompteID FROM Comptes WHERE NumeroCompte LIKE 'INV-202%' OR NumeroCompte LIKE 'SVG-202%'
);

DELETE FROM ComptesRoles WHERE CompteID IN (
    SELECT CompteID FROM Comptes WHERE NumeroCompte LIKE 'INV-202%' OR NumeroCompte LIKE 'SVG-202%'
);

DELETE FROM Comptes WHERE NumeroCompte LIKE 'INV-202%' OR NumeroCompte LIKE 'SVG-202%';

DELETE FROM ContactsClients WHERE ClientID IN (
    SELECT ClientID FROM ClientsAuthentification
    WHERE Email IN ('marceus.jethro@email.ht', 'dr.patrick.marcellus@clinic.ht',
                     'joseph.woldy@university.edu.ht', 'alexandra.dorcean@archidesign.ht',
                     'contact@hoteloasis.ht', 'marie.jethro@designs.ht', 'sophia.marcellus@student.ht')
...
DELETE FROM AdressesClients WHERE ClientID IN (
    SELECT ClientID FROM ClientsAuthentification
    WHERE Email IN ('marceus.jethro@email.ht', 'dr.patrick.marcellus@clinic.ht',
                     'joseph.woldy@university.edu.ht', 'alexandra.dorcean@archidesign.ht',
                     'contact@hoteloasis.ht', 'marie.jethro@designs.ht', 'sophia.marcellus@student.ht')
...
DELETE FROM ClientsAuthentification WHERE Email IN (
    'marceus.jethro@email.ht', 'dr.patrick.marcellus@clinic.ht',
    'joseph.woldy@university.edu.ht', 'alexandra.dorcean@archidesign.ht',
    'contact@hoteloasis.ht', 'marie.jethro@designs.ht', 'sophia.marcellus@student.ht'
);

DELETE FROM ClientsIndividuels WHERE ClientID IN (
    SELECT ClientID FROM ClientsAuthentification
    WHERE Email IN ('marceus.jethro@email.ht', 'dr.patrick.marcellus@clinic.ht',
                     'joseph.woldy@university.edu.ht', 'alexandra.dorcean@archidesign.ht',
                     'marie.jethro@designs.ht', 'sophia.marcellus@student.ht')
);

DELETE FROM ClientsInstitutionnels WHERE NomEntreprise = 'Hotel Oasis SARL';

DELETE FROM Clients WHERE ClientID NOT IN (SELECT ClientID FROM ClientsIndividuels)
    AND ClientID NOT IN (SELECT ClientID FROM ClientsInstitutionnels);

PRINT '✓ Nettoyage terminé';

-- ============================================================================
-- PARTIE 2 : CRÉATION DES CLIENTS AVEC BCRYPT
-- ============================================================================
...
-- Mot de passe: motdepasse123 (hash bcrypt généré)
...
-- ============================================================================
-- PARTIE 4 : AJOUT INSTRUMENTS (Avec TypeInstrumentID corrects)
-- ============================================================================

PRINT 'Vérification et ajout instruments supplémentaires...';
...
-- Vérifier si les instruments existent déjà avant d'insérer
...
PRINT '✓ Instruments vérifiés/ajoutés';
